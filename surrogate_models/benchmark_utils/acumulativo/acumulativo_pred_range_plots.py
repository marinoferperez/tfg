...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Genera plots de rango real vs predicho por batch para acumulativo."
    )
...
def resolver_metricas_modelo(benchmark_root: Path, funcion: str | None, algoritmo: str | None, model: str | None):
    ...
    for ruta in candidatos:
        if ruta.parent.name.startswith(("1-", "21-", "41-", "61-", "81-")):
            continue
...
def usar_escala_log(filas):
    valores = []
    for fila in filas:
        valores.extend([fila["real_p5"], fila["real_p95"], fila["pred_p5"], fila["pred_p95"]])
...
def generar_plot_rangos(ruta_png: Path, filas, *, funcion: str, algoritmo: str, modelo: str):
    ...
    ax.set_xlabel("Batch de entrenamiento acumulado (validado en muestras futuras)")
