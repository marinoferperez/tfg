# implementación del algoritmo SHADE utilizando la librería PYADE.
#
# SHADE (Success-History based Adaptive Differential Evolution) extiende DE
# adaptando F y CR mediante una memoria histórica de valores exitosos.
# La estrategia de mutación empleada es current-to-pbest/1.
#
...
class SHADE:
    # constructor de SHADE
    # --------------------
    # permite construir como objeto el algoritmo, indicando los valores propios
    # de cada uno de los parámetros esenciales de los que depende PYADE.
    #
    # por defecto, pyade asocia los siguientes valores:
    # * population_size = 10 * dim
    # * individual_size = dim
    # * max_evals       = 10000 * dim
    # * memory_size     = 100  (tamaño de la memoria histórica H)
    # * seed            = None
    # * callback        = None
    #
    # F y CR NO son parámetros fijos: SHADE los adapta internamente por individuo
    # en cada generación a partir de la memoria histórica m_cr y m_f.

    def __init__(self, tam_poblacion=None, memory_size=None, max_evals=None, seed=42,
        ...
        # callback de la población inicial (PYADE no lo dispara automáticamente)
    ...
    def evalua_solucion(self, solution):
        # PYADE puede superar max_evals al trabajar con max_iters internamente.
        # Para garantizar el presupuesto exacto devolvemos inf a los individuos
        # que superarían el límite, de modo que no sean seleccionados.
        ...
        if self._buffer_fitness_inicial is not None:
            ...
            if len(self._buffer_fitness_inicial) >= self._tam_poblacion_inicial:
                ...
                self._callback_inicial(
                    ...
                    seed=p.get("seed"),
                    f=None,   # SHADE adapta F por individuo; no hay valor fijo
                    cr=None,  # SHADE adapta CR por individuo; no hay valor fijo
                )
    ...
    # optimize ejecuta SHADE sobre el problema concreto
    ...
    def optimize(self, limites, problem, callback_metricas=None, dataset=None):
        ...
        # get_default_params(dim) devuelve:
        #   population_size, individual_size, max_evals, memory_size, callback, seed, opts
        ...
        # de SHADE depende del estado interno que PYADE expone mediante este callback,
