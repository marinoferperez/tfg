...
% \subsubsection{Descripción del Algoritmo Genético Estacionario (AGE)}

% La selección de progenitores se realiza mediante \textbf{selección por torneo}. En este mecanismo se escoge aleatoriamente un subconjunto de individuos de la población y se selecciona como progenitor aquel con mejor valor de fitness, que en este caso corresponde al menor valor de la función objetivo. En este trabajo se utiliza un tamaño de torneo igual a 2, por lo que cada progenitor se obtiene comparando dos individuos seleccionados al azar. Este procedimiento introduce presión selectiva hacia soluciones de mayor calidad, manteniendo al mismo tiempo cierto componente estocástico en la elección de padres.

% El operador de cruce empleado es \textbf{BLX-\(\alpha\)}, especialmente utilizado en algoritmos genéticos con codificación real. Para cada componente \(j\), si \(\mathbf{x}^{(1)}\) y \(\mathbf{x}^{(2)}\) son los dos progenitores, se define

% \begin{align*}
%     c_{\min} & = \min\{x_j^{(1)}, x_j^{(2)}\}, \\
%     c_{\max} & = \max\{x_j^{(1)}, x_j^{(2)}\}, \\
%     I_j      & = c_{\max} - c_{\min}
% \end{align*}

% El valor de la componente del descendiente se genera entonces como

% \[
%     y_j \sim \mathcal{U}\bigl( c_{\min} - \alpha I_j,\; c_{\max} + \alpha I_j \bigr)
% \]

% donde \(\alpha\) controla el grado de expansión del intervalo de muestreo. Cuando \(\alpha=0\), el descendiente se genera dentro del intervalo delimitado por los progenitores; para valores positivos, el intervalo se amplía, permitiendo explorar regiones próximas pero externas a las soluciones parentales.

% Tras el cruce se aplica una \textbf{mutación gaussiana}, que introduce pequeñas perturbaciones aleatorias sobre las componentes del individuo. Si \(x_j\) representa una componente del descendiente, su valor mutado se calcula como
% \[
%     y_j = x_j + \sigma z \,, \qquad z \sim \mathcal{N}(0,1)
% \]
% donde \(\sigma\) controla la magnitud de la perturbación. Esta mutación contribuye a mantener diversidad en la población y reduce el riesgo de convergencia prematura.

% Finalmente, los hijos generados se evalúan mediante la función objetivo y se selecciona el mejor de ellos. El reemplazo sigue un \textbf{esquema estacionario}: el mejor hijo reemplaza al peor individuo de la población únicamente si mejora su calidad. En términos de la formalización abstracta, en cada iteración se generan dos descendientes candidatos, por lo que \(\lambda = 2\), mientras que el tamaño de la población mantenida es \(\mu = 50\). Por tanto, se cumple que \(\lambda \ll \mu\), característica propia de los esquemas estacionarios. Además, como solo el mejor de los dos hijos puede incorporarse a la población, la renovación efectiva se produce de forma parcial y gradual. De este modo, la población no se reemplaza por completo en cada iteración, sino que evoluciona mediante incorporaciones puntuales de descendencia.

\subsection{Differential Evolution (DE)}

La segunda de las técnicas seleccionadas para estudiar en este trabajo es  \textit{Differential Evolution} (\textbf{DE}), introducido por Rainer Storn y Kenneth Price como una metaheurística evolutiva orientada a problemas de optimización continua. Una de sus primeras validaciones competitivas se presentó en el \textit{IEEE International Conference on Evolutionary Computation} de 1996 (ICEC'96), una de las conferencias precursoras del actual \textit{IEEE Congress on Evolutionary Computation} (CEC). En dicho trabajo, los autores aplicaron DE sobre las funciones de optimización continua propuestas en la competición del congreso \cite{storn1996icec}. Posteriormente, el algoritmo fue formalizado en mayor profundidad por los mismos autores \cite{storn1997de}.
...
En la literatura, \textit{Differential Evolution} no se presenta como un único esquema fijo, sino como una familia de estrategias que combinan distintas formas de mutación y cruce. Una notación habitual para describir una variante concreta es \texttt{DE/x/y/z} \cite{price2005differential}, donde:
...
\input{figuras/esquema_reinicio/reinicio_elitista.tex}
...
En estas expresiones, \(W^{(\ell)}\) y \(\mathbf{b}^{(\ell)}\) representan los pesos y sesgos de la capa \(\ell\), mientras que \(\sigma^{(\ell)}\) determina su función de activación. La composición sucesiva de estas transformaciones permite aproximar relaciones no lineales complejas entre las variables de entrada y la salida \cite{hornik1989multilayer}.
...
% En el caso de HGB, la discretización previa de las variables continuas mediante histogramas permite acelerar el entrenamiento y reducir el coste computacional. XGBoost, por su parte, incorpora regularización explícita y estrategias eficientes de construcción de árboles, lo que le confiere una combinación competitiva de precisión y robustez. Como surrogates, ambos métodos resultan especialmente útiles en problemas tabulares con relaciones no lineales complejas, aunque, al igual que otros enfoques basados en árboles, presentan limitaciones para extrapolar fuera de las regiones observadas.




% Aunque evalúes el modelo midiendo cómo ordena las soluciones (mediante coeficientes como Spearman o Kendall), el modelo matemático subyacente sigue aprendiendo un mapeo de un espacio continuo a una variable de salida continua: R
% D
% →R (la función de fitness). Vas a entrenar regresores clásicos (como RBF o Random Forests de regresión) que devuelven números reales, no clasificadores ni modelos de ordenación pura (Learning to Rank basándose en comparaciones por pares).
% Explicar este contraste entre lo que predice el modelo (un valor continuo) y lo que nos interesa de su predicción (el orden relativo) enriquecerá enormemente tu marco teórico. A los tribunales de la ETSIIT les entusiasma ver que el alumno entiende que en optimización evolutiva no importa si el metamodelo se equivoca en el tercer decimal, siempre y cuando sea capaz de identificar correctamente qué individuo es mejor que otro para guiar la selección.


\section{Técnicas Subrogadas clásicas de referencia}
\label{sec:tecnicas_subrogadas_clasicas}

%% motivacion de incluirlas: explicar que, aunque el trabajo se centra en modelos de aprendizaje automático, se consideran también técnicas clásicas de modelado subrogado como referencia posterior

%% diferencias respecto a los modelos de ml: no presentarlas como otros regresores mas, sino como métodos tradicionalmente diseñados para aproximar funciones objetivo costosas en optimizacion, tal y como se comentaba en el capitulo 3

%% kriging / procesos gaussianos:
% - idea general: modelar la funcion como una realizacion de un proceso estocastico
% - prediccion con media e incertidumbre
% - utilidad en optimizacion surrogate y smbo
% - limitaciones: coste cubico aproximado con el numero de muestras,
% sensibilidad al kernel/covarianza.

%% pce:
% - idea general: aproximar la respuesta mediante una expansion en polinomios ortogonales
% - adecuado cuando se quiere representar la funcion mediante una base global
% - limitaciones: puede degredarse en paisajes muy irregulares o altamente multimodales

%% papel en este tfg: dejar claro que no forman parte del cribado inicial de modelos ml, sino que se reservan como referencia clasica para una comparacion posterior con el subrogado seleccionado.
...
\textit{Kriging} es una técnica clásica de modelado subrogado utilizada para aproximar funciones costosas a partir de un conjunto limitado de observaciones. Su aplicación al diseño y análisis de experimentos computacionales fue consolidada por Sacks et al. \cite{sacks1989}. Posteriormente, adquirió especial relevancia en optimización mediante métodos como \textit{Efficient Global Optimization} (EGO) \cite{jones1998}.

Este método puede formularse mediante \textbf{procesos gaussianos}. La función objetivo se modela como la suma de una tendencia global y un proceso estocástico correlacionado:
...
Por tanto, el tamaño de la base puede crecer rápidamente. Además, una expansión polinómica global puede perder eficacia ante funciones altamente irregulares o con variaciones locales pronunciadas. Existen formulaciones dispersas que tratan de mitigar estas limitaciones seleccionando únicamente los términos más relevantes \cite{blatman2011adaptive}.
...
% \cite{stone1974crossvalidatory,geisser1975predictive,kohavi1995study}.
...
\input{figuras/estrategias/acumulativa.tex}
