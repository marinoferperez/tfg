...
\begin{table}[H]
    ...
    \input{tablas/tacolab/age_tacolab_ranking.tex}
...
\begin{table}[H]
    ...
    \setlength{\tabcolsep}{6pt}
    \renewcommand{\arraystretch}{0.95}
    \input{tablas/tacolab/de_tacolab_ranking.tex}
...
\begin{table}[H]
    ...
    \setlength{\tabcolsep}{6pt}
    \renewcommand{\arraystretch}{0.95}
    \input{tablas/tacolab/shade_tacolab_ranking.tex}
...
\input{tablas/resumen_global_por_estrategia_sin_svr.tex}
...
Para completar esta comparación y dotar de rigor cuantitativo las observaciones, la Tabla~\ref{tab:comparativa_modelos_bloques_informativos} incorpora las métricas agregadas de error y coste computacional definidas en el Capítulo~\ref{chap:metodologia_experimental}.

\input{tablas/comparativa_modelos_offline}
...
A partir del protocolo descrito en la Sección~\ref{sec:diseno_online}, se evalúan los valores candidatos de cada parámetro sobre el subconjunot de funciones y semillas definido.

%% primero cooldown ya que depende del reinicio

%% luego retrain_ratio, porque controla actualizacion/coste del modelo

%% p, porque regula la agresivididad real del filtro y es el mas importante
...
%% comparacion contrao p = 0 -> sin intervencion efectiva del subrogado
...
% - resultado de la seleccion offline del modelo (omitible)
% - configuracion del modelo finalmente usado online (omitible)
% - piloto de cooldown
% - piloto de retrain_ratio
% - piloto de p
...
% - discusion: mejora, coste añadido, rechazo de candidatos, comportamietno por metaheuristica.





% A partir del protocolo descrito en la Sección~X, se evalúan los valores candidatos de cada parámetro. Los resultados muestran que cooldown=100, retrain_ratio=0.50 y p=0.50 ofrecen el mejor compromiso entre error final, ranking medio y coste computacional, por lo que se adoptan como configuración final para la evaluación online completa.
...
% Con el modelo seleccionado y ajustado, esta sección conti en evaluar su integración dentro del bucle de optimización.

% 4. [Tabla comparativa reinicio vs sin reinicio] → validación de que el escenario evaluado es el correcto
...
%% en los scripts tenemos un flag para introducir o no enfriamiento tras un reinicio.
%% la idea a la hora de documentar los experiemntos es comenzar sin enfriamiento y posteriormente justificar la necesidad de ese enfriamiento. ais lo hacemos mas largo.

%% es una decision experimental
...
% Justificación de la configuración homogénea seleccionada.
% Comparación final entre:
% metaheurística base;
% metaheurística con reinicio;
% metaheurística con reinicio + subrogado online.
% Análisis por metaheurística.
% Análisis por función o familia CEC2017.
% Evaluación del ahorro de evaluaciones reales.
% Discusión de cuándo ayuda y cuándo perjudica.
% Conclusión final sobre si la integración online es útil.

% \subsection{Ajuste piloto de los parámetros online}
% \subsection{Comparación frente a las metaheurísticas base}
% \subsection{Impacto sobre la convergencia y el uso de evaluaciones reales}
% \subsection{Análisis por metaheurística}
% \subsection{Conclusiones de la integración online}
