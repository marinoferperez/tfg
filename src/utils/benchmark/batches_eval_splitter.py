"""Construccion de casos de entrenamiento/validacion por batches temporales del 20%."""
...
def muestrear_validacion_siguiente(validacion_idx, n_muestras_val, random_state, seed, batch_id):
    validacion_idx = np.asarray(validacion_idx, dtype=np.int64)

    if validacion_idx.size < n_muestras_val:
        raise ValueError(
            f"No hay suficientes muestras en el bloque de validación: "
            f"available={validacion_idx.size}, required={n_muestras_val}"
        )
...
def obtener_batch_validacion_siguiente(batches, idx_batch):
    siguientes = list(batches[idx_batch + 1 :])
    return siguientes[:1]
...
def _construir_caso_temporal(
    *,
    ...
    return {
        ...
        "batch_validacion": int(batch_validacion),
        "train_idx": train_idx,
        "val_idx": val_idx,
...
def construir_casos_no_acumulativos(dataset, random_state=42):
    ...
    for idx_batch, batch in enumerate(batches[:-1]):
        ...
        batches_validacion = obtener_batch_validacion_siguiente(
            batches,
            idx_batch,
        )
        validacion_idx = np.concatenate(
            [np.asarray(item["indices"], dtype=np.int64) for item in batches_validacion]
        )
        if validacion_idx.size == 0:
            continue

        val_idx = muestrear_validacion_siguiente(
            validacion_idx,
            n_muestras_val_fija,
        ...
        casos.append(
            _construir_caso_temporal(
                ...
                batch_validacion=batches_validacion[0]["batch_id"],
                train_idx=train_idx,
                val_idx=val_idx,
                eval_id=eval_id,
            )
