...
class SHADECEC2017Online(SHADECEC2017):
    ...
    def optimize(self, funcid, dim, seed=42, lib_path=None, algname="shade_online", registrar_metricas=False, ruta_metricas=None, run_id=None, cec_workdir=None, guardar_decisiones_subrogado=False, guardar_reinicios_detalle=False):
        """
        ...
        try:
            ...
            mejor_sol, mejor_fitness = self.shade.optimize(limites=problema.get_bounds(), problema=problema, controlador_subrogado=controlador_subrogado, callback_metricas=callback_metricas)
            ...
            if registrar_metricas:
                ...
                if ruta_base is not None:
                    ...
                    ficheros_metricas = guardar_metricas_deap(recolector, ruta_base=ruta_base, metadata={"algoritmo": "shade_online", "problema": "cec2017", "funcid": int(funcid), "dim": int(dim), "seed": int(seed), "tam_poblacion": config.get("population_size"), "max_evals_objetivo": evals_objetivo, "memory_size": config.get("memory_size"), "evals_reales": evals_reales, "evals_fuera_presupuesto": evals_fuera_presupuesto, "hubo_fuera_presupuesto": hubo_fuera_presupuesto, **metadata_reinicios, **resumen_online})
