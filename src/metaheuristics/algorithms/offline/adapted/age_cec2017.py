...
class GeneticStationaryCEC2017:
    ...
    def optimize(self, funcid, dim, seed=42, algname="age", lib_path=None, registrar_metricas=False, ruta_metricas=None, run_id=None, cec_workdir=None, guardar_reinicios_detalle=False):
        ...
        Retorna un dict con mejor_sol, mejor_fitness, mejor_error y, si registrar_metricas=True, las rutas a los ficheros generados.
        """
        seed = int(seed)
        ...
        try:
            ...
            if registrar_metricas:
                ...
                dataset = SurrogateDataset(algoritmo="age", problema="cec2017", run_info={"funcid": int(funcid), "dim": int(dim)})

            # ejecución del algoritmo
            mejor_sol, mejor_fitness = self.age.optimize(limites=problema.get_bounds(), problema=problema, callback_metricas=callback_metricas, dataset=dataset)
            ...
            if registrar_metricas:
                ...
                if ruta_metricas is not None:
                    ...
                    ficheros_metricas = guardar_metricas_deap(recolector, ruta_base=ruta_base, metadata={"algoritmo": "age", "problema": "cec2017", "funcid": int(funcid), "dim": int(dim), "seed": int(seed), "tam_poblacion": int(self.age.tam_poblacion), "prob_cruce": float(self.age.prob_cruce), "prob_mutacion": float(self.age.prob_mutacion), "tam_torneo": int(self.age.tam_torneo), "max_evals": int(self.age.max_evals) if self.age.max_evals else None, "sigma": float(self.age.sigma), "alpha": float(self.age.alpha), **metadata_reinicios})
