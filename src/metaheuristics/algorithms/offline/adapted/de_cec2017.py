    ...
    def optimize(self, funcid, dim, seed=42, lib_path=None, algname="de", registrar_metricas=False, ruta_metricas=None, run_id=None, cec_workdir=None, guardar_reinicios_detalle=False):
        ...
        try:
            ...
            if registrar_metricas:
                ...
                dataset = SurrogateDataset(algoritmo="de", problema="cec2017", run_info={"funcid": int(funcid), "dim": int(dim)})
            ...
            mejor_sol, mejor_fitness = self.de.optimize(limites=problema.get_bounds(), problema=problema, callback_metricas=callback_metricas, dataset=dataset)
            ...
            if registrar_metricas:
                ...
                if ruta_metricas is not None:
                    ...
                    ficheros_metricas = guardar_metricas_deap(recolector, ruta_base=ruta_base, metadata={"algoritmo": "de", "problema": "cec2017", "funcid": int(funcid), "dim": int(dim), "seed": int(seed), "tam_poblacion": config.get("population_size"), "max_evals_objetivo": evals_objetivo, "f": config.get("f"), "cr": config.get("cr"), "cross": config.get("cross"), "evals_reales": evals_reales, "evals_fuera_presupuesto": evals_fuera_presupuesto, "hubo_fuera_presupuesto": hubo_fuera_presupuesto, **metadata_reinicios})
